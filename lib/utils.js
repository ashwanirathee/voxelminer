...
import { gl, shaderVars, canvas, camera} from "./global.js";
...
export function getAttrib(gl, program, name, targetObj = null) {
  ...
  if (location < 0) {
    console.warn(`⚠️ Failed to get the attribute location of ${name}`);
  }
...
export function getUniform(gl, program, name, targetObj = null) {
  ...
  if (!location) {
    console.warn(`⚠️ Failed to get the storage location of ${name}`);
  }
  if (targetObj && name in targetObj) {
    targetObj[name] = location;
  }
 ...
 */
export function connectVariablesToGLSL(gl, VSHADER_SOURCE, FSHADER_SOURCE) {
  // Initialize shaders
  ...
  // Attributes
  shaderVars.attribs.a_Position = getAttrib(gl, gl.program, "a_Position");
  shaderVars.attribs.a_UV = getAttrib(gl, gl.program, "a_UV");
  shaderVars.attribs.a_Normal = getAttrib(gl, gl.program, "a_Normal");
  shaderVars.attribs.a_CameraPos = getUniform(gl, gl.program, "a_CameraPos");

  // Global Uniforms
  shaderVars.attribs.u_FragColor = getUniform(gl, gl.program, "u_FragColor");
  shaderVars.attribs.u_whichTexture = getUniform(gl, gl.program, "u_whichTexture");
  shaderVars.attribs.u_ModelMatrix = getUniform(gl, gl.program, "u_ModelMatrix");
  shaderVars.attribs.u_NormalMatrix = getUniform(gl, gl.program, "u_NormalMatrix");
  shaderVars.attribs.u_ViewMatrix = getUniform(gl, gl.program, "u_ViewMatrix");
  shaderVars.attribs.u_ProjectionMatrix = getUniform(gl, gl.program, "u_ProjectionMatrix");
  shaderVars.attribs.u_Sampler0 = getUniform(gl, gl.program, "u_Sampler0");
  shaderVars.attribs.u_Sampler1 = getUniform(gl, gl.program, "u_Sampler1");
  shaderVars.attribs.u_Sampler2 = getUniform(gl, gl.program, "u_Sampler2");
  shaderVars.attribs.u_LightColor = getUniform(gl, gl.program, "u_LightColor");
  shaderVars.attribs.u_lightPos = getUniform(gl, gl.program, "u_lightPos");
  shaderVars.attribs.u_light2Pos = getUniform(gl, gl.program, "u_light2Pos");
  shaderVars.attribs.u_lightStatus = getUniform(gl, gl.program, "u_lightStatus");
  shaderVars.attribs.u_light2Status = getUniform(gl, gl.program, "u_light2Status");

  // Identity Model Matrix
  const identityM = new Matrix4();
  gl.uniformMatrix4fv(shaderVars.attribs.u_ModelMatrix, false, identityM.elements);
}
