...
debugLog(debugkey, "Loading texture...");
...
export function initTextures(gl, textureDataList) {
  for (let { url, unit, sampler } of textureDataList) {
    ...
    const totalTextures = textureDataList.length;

    image.onload = () => {
      sendTextureToUnit(image, gl, unit, sampler);
      texturesLoaded++;
      if (texturesLoaded === totalTextures) {
        debugLog(debugkey, "All textures loaded.");
      }
    };
  }
