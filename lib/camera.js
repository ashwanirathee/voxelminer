...
import { debugLog, degToRad, radToDeg } from "./utils.js";
...
export class Camera {
  ...
  constructor(speed, sensitivity, g_eye, g_at, g_up, field_angle, asp_ratio, near_plane, far_plane, gl, scene) {
    ...
    this.type = CameraType.FPS_CAMERA; // default FPS
    ...
    this.pitch = 0.0;
    this.yaw = -90.0;
    this.orbitRadius = this.eye.clone().sub(this.at).magnitude();
  ...
  changeSensitivity(newSensitivity) {
    this.sensitivity = Math.max(0.01, Math.min(newSensitivity, 2.0)); 
  }

  /**
   * Changes the arcball radius value of the camera by a specified value.
   * @param {number} newRadiusOffset - The new offset to be added to the radius.
   */
  changeOrbitRadiusByOffset(newRadiusOffset) {
    this.orbitRadius = Math.max(0.1, this.orbitRadius+this.sensitivity*newRadiusOffset);
    this.panCamera(0,0);
  }
  ...
  changeCameraType(cameraType) {
    ...
    if(cameraType===CameraType.ARCBALL_CAMERA){ // TODO: maybe pass this as arg or something
      this.changeAt(0,1,0);
    }
    const front = this.at.clone().sub(this.eye).normalize();
    this.yaw   = radToDeg(Math.atan2(front.elements[2], front.elements[0]));
    this.pitch = radToDeg(Math.asin(front.elements[1]));
    if (cameraType === CameraType.ARCBALL_CAMERA) {
      this.orbitRadius = this.eye.clone().sub(this.at).magnitude();
    }
    this.updateViewMatrix();
  }
  ...
  panArcballCamera(direction) {
    this.eye = this.at.clone().add(direction.mul(this.orbitRadius));
    this.updateViewMatrix();
  }
