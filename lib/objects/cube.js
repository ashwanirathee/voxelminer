...
import { gl, scene, shaderVars, renderer } from "../global.js";
...
export class Cube {
   ...
   */
  render() {
    let tex = scene.normalControllerState ? -3 : this.textureAtlasNum;

    // passs data to u_whichTexture
    gl.uniform1i(shaderVars.attribs.u_whichTexture, tex);
    gl.uniform4f(shaderVars.attribs.u_FragColor, ...this.color);
    gl.uniformMatrix4fv(shaderVars.attribs.u_ModelMatrix, false, this.matrix.elements);

    this.normalMatrix.set(this.matrix).invert().transpose();
    gl.uniformMatrix4fv(shaderVars.attribs.u_NormalMatrix, false, this.normalMatrix.elements);
